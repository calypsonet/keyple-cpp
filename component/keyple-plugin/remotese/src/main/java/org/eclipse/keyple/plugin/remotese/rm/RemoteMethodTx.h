...
//JAVA TO C++ CONVERTER TODO TASK: Native C++ does not allow initialization of static non-const/integral fields in their declarations - choose the conversion option for separate .h and .cpp files:
                        ...
                        public:
                            ...
                            T get() throw(KeypleRemoteException) {
                                ...
                                std::shared_ptr<RemoteMethodTx> * const thisInstance = shared_from_this();

                                std::shared_ptr<Thread> asyncGet = std::make_shared<ThreadAnonymousInnerClass>(shared_from_this(), thisInstance);

                                try {
                                    lock = std::make_shared<CountDownLatch>(1);
                                    logger->trace(std::string("") + "" + "Set callback on RemoteMethodTx {} {}", this->getClass().getCanonicalName(), this->hashCode());
                                    asyncGet->start();
                        ...
                        private:
                            ...
                            private:
                                ...
                                std::shared_ptr<RemoteMethodTx> thisInstance;

                            public:
                                ThreadAnonymousInnerClass(std::shared_ptr<RemoteMethodTx<std::shared_ptr<T>>> outerInstance, std::shared_ptr<RemoteMethodTx> thisInstance);
